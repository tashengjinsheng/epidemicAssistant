...
function getMessage(side, str) {
    ...
    var template = `  <div class="message-item massage-item-${side}">
            <img class="avata" src="./image/${side == 'left' ? 'bot' : 'user'}.jpg" alt="头像">
...
function getMessag(msObj) {//利用ES6字符串模板的方法 生成DOM模板
    ...
    <div class="message-item massage-item-right  js-to-select"data.score=${msObj.score}>
            <img class="avata" src="./image/user.jpg" alt="头像">
            <div class="massage-bue">
                <p>${msObj.text}</p>
            </div>
        </div>
...
function showResult() {
    setTimeout(() => {
        // 显示左边最后的对话
        var resultObj = getResultByScore(score)
        // 延时 1s 显示结果窗口
...
$('.inputname').addEventListener("keyup", function(event) {
     ...
     if (event.keyCode === 13) {
        // alert('hello');
        var text_field = document.getElementById('inputtext');
        var message = text_field.value;
        appendMessage('right', message);
        text_field.value = "";
        jQuery.ajax({
              type: "post",
